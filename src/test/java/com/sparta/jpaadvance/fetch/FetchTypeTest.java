...
public class FetchTypeTest {
    ...
    @Test
    @Transactional
    @DisplayName("아보카도 피자 조회")// 지연 로딩은 Transactional을 걸어줘야 한다!
    void test1() {
    ...
    @Test
    @Transactional
    @DisplayName("Robbie 고객 조회")// 지연 로딩은 Transactional을 걸어줘야 한다!
    void test2() {
        User user = userRepository.findByName("Robbie");
        System.out.println("user.getName() = " + user.getName());

        System.out.println("Robbie가 주문한 음식 이름 조회");
        // getFoodList를 하는 순간 sql을 실행시킴
    ...
    @Test
    @DisplayName("Robbie 고객 조회 실패") // 지연 로딩은 Transactional을 걸어줘야 한다!
    void test3() {
        User user = userRepository.findByName("Robbie");
        System.out.println("user.getName() = " + user.getName());
